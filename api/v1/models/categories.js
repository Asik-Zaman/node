...
let getCatergoryList = async () => {
  return new Promise((resolve, reject) => {
    connectionMYSQLGeneral.query(queries.getCategoryList(), (error, result) => {
...
let addNewCategories = async (info) => {
  return new Promise((resolve, reject) => {
    connectionMYSQLGeneral.query(
      queries.addNewCategories(),
      [info],
      (error, result, fields) => {
        if (error) reject(error);
        else resolve(result);
...
let getDataByWhereCondition = async (
  ...
  let connection = connectionMYSQLGeneral;
